...
kf_3d = KF3D(np.ones((9,1), dtype=np.float32), 0.03, 0.03, 0.03, 0.05, 1)
...
def when_bounding_boxes_refresh():
    ...
    if DEPTH_COMPATIBLE:
        ...
        if (best_bounding_box != None):
            ...
            print("acceleration mag:", np.linalg.norm(np.array([kf_3d.kalman.statePost[2][0], kf_3d.kalman.statePost[5][0], kf_3d.kalman.statePost[8][0]])))  
            past_time = time.time()
