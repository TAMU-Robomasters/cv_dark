...
def generate_image(fps=0):
    ...
    if len(bounding_boxes) > 0:
        ...
        if target_status == TargetStatus.TARGET_FOUND:
        ...
        if display_kf_prediction:
            if depth_compatible and target_status != TargetStatus.TARGET_NONE:
            ...
            elif not depth_compatible and target_status != TargetStatus.TARGET_NONE:
    ...
    if depth_compatible:
        ...
        if display_kf_prediction and target_status != TargetStatus.TARGET_NONE:
            if str(type(target_3d_kinematic_state)) != str(type(None)):
                image.add_text(text=f"target_3d_state: P {float(target_3d_kinematic_state[0]):.2f}, {float(target_3d_kinematic_state[3]):.2f}, {float(target_3d_kinematic_state[6]):.2f}",  location=(x_location, y_location)); y_location += 50
                image.add_text(text=f"target_3d_state: V {float(target_3d_kinematic_state[1]):.2f}, {float(target_3d_kinematic_state[4]):.2f}, {float(target_3d_kinematic_state[7]):.2f}",  location=(x_location, y_location)); y_location += 50
